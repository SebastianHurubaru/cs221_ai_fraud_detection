...
def summarise_by_sum(doc, variable_names, take_the_latest_avaiable_date=False):
	"""
	...
	if take_the_latest_avaiable_date is True:
...
def summarise_by_priority(doc, variable_names, take_the_latest_avaiable_date=False):
	"""
	Takes a document (dict) after extraction, and tries to extract
	a summary variable relating to the financial state of the enterprise
...
def process_account_data(data):
	...
	# Fetch all the marked elements of the document
	try:
		doc['elements'] = scrape_elements(soup, '')
	except Exception as e:
		doc['parsed'] = False
		doc['Error'] = e
